...
void Frame::ensureNativeWindow(){if(!nativeWindow_){nativeWindow_=::jxx::NEW<::jxx::gui::internal::WxFramePeer>(title_);nativeWindow_->setBounds(getX(),getY(),getWidth(),getHeight());configureNativeEvents();displayable_=true;}}
...
::jxx::Ptr<MenuBar> Frame::getMenuBar() const{return menuBar_;} void Frame::setMenuBar(const ::jxx::Ptr<MenuBar>& value){menuBar_=value;} void Frame::remove(const ::jxx::Ptr<MenuComponent>& component){if(menuBar_&&component==::jxx::CAST<MenuComponent>(menuBar_))menuBar_.reset();}
}
