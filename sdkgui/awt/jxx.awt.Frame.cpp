...
void Frame::ensureNativeWindow(){if(!nativeWindow_){nativeWindow_=::jxx::NEW<::jxx::gui::internal::WxFramePeer>(title_);nativeWindow_->setBounds(getX(),getY(),getWidth(),getHeight());nativeWindow_->setMenuBar(menuBar_);configureNativeEvents();::jxx::CAST<::jxx::gui::internal::WxFramePeer>(nativeWindow_)->installComponents(::jxx::CAST<Container>(thisPtr()));displayable_=true;}}
