...
void WxFramePeer::installComponents(const ::jxx::Ptr<::jxx::awt::Container>& container){if(!frame_||!container)return;buildChildren(frame_,container);}
void WxFramePeer::buildChildren(wxWindow* parent,const ::jxx::Ptr<::jxx::awt::Container>& container){for(::jxx::lang::jint i=0;i<container->getComponentCount();++i){auto component=container->getComponent(i);if(!component||component->nativeComponent_)continue;wxWindow* native=nullptr;if(auto button=::jxx::CAST<::jxx::awt::Button>(component)){auto* control=new wxButton(parent,wxID_ANY,nativeText(button->getLabel()));auto peer=::jxx::NEW<WxComponentPeer>(control);std::weak_ptr<::jxx::awt::Button> weak=button;peer->setActionCallback([weak](){if(auto owner=weak.lock())owner->fireActionPerformed();});component->setNativeComponentInternal(peer);native=control;}else if(auto label=::jxx::CAST<::jxx::awt::Label>(component)){long style=label->getAlignment()==::jxx::awt::Label::RIGHT?wxALIGN_RIGHT:(label->getAlignment()==::jxx::awt::Label::CENTER?wxALIGN_CENTRE_HORIZONTAL:wxALIGN_LEFT);auto* control=new wxStaticText(parent,wxID_ANY,nativeText(label->getText()),wxDefaultPosition,wxDefaultSize,style|wxST_NO_AUTORESIZE);component->setNativeComponentInternal(::jxx::NEW<WxComponentPeer>(control));native=control;}else if(auto field=::jxx::CAST<::jxx::awt::TextField>(component)){long style=wxTE_PROCESS_ENTER;if(field->echoCharIsSet())style|=wxTE_PASSWORD;auto* control=new wxTextCtrl(parent,wxID_ANY,nativeText(field->getText()),wxDefaultPosition,wxDefaultSize,style);control->SetEditable(field->isEditable());auto peer=::jxx::NEW<WxComponentPeer>(control);std::weak_ptr<::jxx::awt::TextField> weak=field;peer->setActionCallback([weak,control](){if(auto owner=weak.lock()){owner->setText(::jxx::NEW<::jxx::lang::String>(control->GetValue().ToUTF8().data()));owner->fireActionPerformed();}});component->setNativeComponentInternal(peer);native=control;}else if(auto child=::jxx::CAST<::jxx::awt::Container>(component)){auto* panel=new wxPanel(parent,wxID_ANY);component->setNativeComponentInternal(::jxx::NEW<WxComponentPeer>(panel));native=panel;buildChildren(panel,child);}if(native)native->Show(component->isVisible());}}
}
