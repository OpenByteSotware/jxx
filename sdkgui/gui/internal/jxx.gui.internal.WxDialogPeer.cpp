...
namespace jxx::gui::internal
{
              ...
              : ::jxx::NEW<::jxx::lang::String>(""))
    {
        ...
        {
            send(event.IsShown()
                ? ::jxx::awt::event::WindowEvent::WINDOW_OPENED
        ...
        dialog_->Bind(wxEVT_ACTIVATE, [this](wxActivateEvent& event)
        {
            send(event.GetActive()
                ? ::jxx::awt::event::WindowEvent::WINDOW_ACTIVATED
    ...
    void WxDialogPeer::hide()
    {
        if (dialog_ == nullptr) return;
        if (dialog_->IsModal()) dialog_->EndModal(wxID_OK);
        else dialog_->Show(false);
    }
    void WxDialogPeer::destroy()
    {
        if (dialog_ == nullptr) return;
        auto* value = dialog_;
        dialog_ = nullptr;
    ...
    void WxDialogPeer::setBounds(::jxx::lang::jint x, ::jxx::lang::jint y,
        ::jxx::lang::jint width, ::jxx::lang::jint height)
    { if (dialog_ != nullptr) dialog_->SetSize(x, y, width, height); }
    void WxDialogPeer::setTitle(const ::jxx::Ptr<::jxx::lang::String>& title)
    {
        title_ = title != nullptr ? title : ::jxx::NEW<::jxx::lang::String>("");
        if (dialog_ != nullptr) dialog_->SetTitle(nativeText(title_));
    }
    ...
    void WxDialogPeer::installComponents(const ::jxx::Ptr<::jxx::awt::Container>& container)
    { if (dialog_ != nullptr && container != nullptr) buildChildren(dialog_, container); }

    void WxDialogPeer::buildChildren(wxWindow* parent,
        const ::jxx::Ptr<::jxx::awt::Container>& container)
    {
        for (::jxx::lang::jint index = 0; index < container->getComponentCount(); ++index)
        {
            ...
            {
                auto* control = new wxTextCtrl(parent, wxID_ANY, nativeText(field->getText()),
            ...
            else if (const auto child = ::jxx::CAST<::jxx::awt::Container>(component))
            {
                ...
                component->setNativeComponentInternal(::jxx::NEW<WxComponentPeer>(panel));
                native = panel; buildChildren(panel, child);
            }
