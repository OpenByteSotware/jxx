...
using jxx::lang::Formatter;
using jxx::lang::String;
using jxx::lang::FormatArg;
...
String Formatter::format(const String& fmt, const std::vector<FormatArg>& args) {
    out_.clear();
    ...
    for (size_t i=0; i<f.size(); ++i) {
        ...
        switch (conv) {
            ...
            case 'e': case 'E': case 'f': case 'g': case 'G': case 'a': case 'A': {
                ...
                if (conv=='e'||conv=='E') oss << std::scientific << v; else if (conv=='f') oss << std::fixed << v; else if (conv=='a'||conv=='A') { oss << std::hexfloat << v; } else oss << v;
...
Formatter& Formatter::setLocale(const std::string& locale_name) {
    try { locale_ = std::locale(locale_name.c_str()); } catch (...) { locale_ = std::locale(""); }
    return *this;
}
