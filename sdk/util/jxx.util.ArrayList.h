...
public:
    template <typename> friend class SubList;
    ...
    explicit ArrayList(jxx::Ptr<Collection<E>> c)
        : elementData(), size_(0) {
        if (c == nullptr) {
    ...
    virtual jbool contains(jxx::Ptr<E> o) override {
        return indexOf(o) >= 0;
    }
    ...
    virtual jbool remove(jxx::Ptr<E> o) override {
        jint i = indexOf(o);
        if (i >= 0) {
            remove(i);
            return true;
    ...
    virtual jbool addAll(jxx::Ptr<Collection<E>> c) override {
    ...
    virtual jbool removeIf(jxx::Ptr<function::Predicate<E>> filter) override {
        if (filter == nullptr) {
            throw NullPointerException();
        }
        jbool removed = false;
    ...
    virtual void sort(jxx::Ptr<Comparator<E>> c) override {
        if (c == nullptr) {
            throw NullPointerException();
        }
        const jint expected = this->modCount;
...
private:
    ...
    void grow(jint minCapacity) {
        ...
        auto newData = copyArray(newCapacity);
        elementData = newData;
        ++this->modCount;
    }
