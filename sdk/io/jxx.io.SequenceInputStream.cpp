...
int SequenceInputStream::read(jxx::lang::ByteArray& b, int off, int len){ if(len==0) return 0; int total=0; while(len>0 && idx<seq.size()){ auto r=seq[idx]->read(b,off,len); if(r==-1){ ++idx; continue; } off+=r; len-=r; total+=r; if(r>0) break; } return (total>0)? total : -1; }
