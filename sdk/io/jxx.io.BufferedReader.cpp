...
int BufferedReader::fill(){ pos=0; limit=0; int n=in->read(buf.data(),0,(int)buf.size()); if(n<=0) return -1; limit=n; return n; }
...
std::u16string BufferedReader::readLine(){ 
	...
	if(ch<0) return seenAny?line:std::u16string(); seenAny=true; if(ch==''){ break; } else if(ch==''){ int next=read();
	if(next!='' && next>=0){ pos--; } break; } else { line.push_back((jxx::lang::jchar)ch);} } return line; }
