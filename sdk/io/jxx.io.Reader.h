...
public:
    virtual ~Reader() = default; virtual int read() = 0; 
    virtual int read(jxx::lang::jchar* buf, int off, int len){ if(!buf||off<0||len<0) throw IOException("Reader.read: invalid args"); 
    int i=0; for(;i<len;++i){ int c=read(); if(c<0) break; buf[off+i]=(jxx::lang::jchar)c;} return (i==0)?-1:i; } 
