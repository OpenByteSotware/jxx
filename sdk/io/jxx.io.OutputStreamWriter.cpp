...
void OutputStreamWriter::write(const jxx::lang::jchar* buf, int off, int len){
	if(!buf||off<0||len<0) throw IOException("OutputStreamWriter.write: invalid args"); 
std::u16string s(buf+off, buf+off+len); auto enc=UTF8::encode(s); out->write(enc->data(), enc->length);
} 

void OutputStreamWriter::write(const std::u16string& s){
	if(s.empty()) return;
auto enc=UTF8::encode(s); 
out->write(enc->data(), enc->length);} 
