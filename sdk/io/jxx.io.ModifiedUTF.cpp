...
if((b2&0xC0)!=0x80) throw UTFDataFormatException("Invalid modified UTF-8"); uint16_t ch=(uint16_t)(((b&0x1F)<<6)|(b2&0x3F)); 
out.push_back((char16_t)ch);} else if((b&0xF0)==0xE0){ if(i+1>=bytes.size()) throw UTFDataFormatException("Invalid modified UTF-8");
uint8_t b2=bytes[i++], b3=bytes[i++]; if(((b2&0xC0)!=0x80)||((b3&0xC0)!=0x80)) throw UTFDataFormatException("Invalid modified UTF-8");
uint16_t ch=(uint16_t)(((b&0x0F)<<12)|((b2&0x3F)<<6)|(b3&0x3F)); out.push_back((char16_t)ch);} else { throw UTFDataFormatException("Invalid modified UTF-8 leading byte"); } }
return out; }
}}
