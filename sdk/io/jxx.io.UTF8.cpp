...
jxx::lang::ByteArray UTF8::encode(const std::u16string& s){
...
std::u16string UTF8::decode(const jxx::lang::ByteArray bytes)
{
	...
	{ 
		...
		uint8_t bx= (*bytes)[i++]; 
		if((bx&0xC0)!=0x80){ cp=0xFFFD; break;} cp=(cp<<6)|(bx&0x3F);} 
		if(cp<=0xFFFF){ out.push_back((char16_t)cp);} else { cp-=0x10000; char16_t hi=(char16_t)(0xD800|((cp>>10)&0x3FF));
		char16_t lo=(char16_t)(0xDC00|(cp&0x3FF)); out.push_back(hi);
