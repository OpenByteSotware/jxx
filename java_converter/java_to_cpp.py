...
Java → C++17 transpiler (AST-based via `javalang`) with:
  ...
  • Import-derived header includes:
      - import java.io.*      →  #include "jxx.io.h"
      - import java.util.Map  →  #include "jxx.util.Map.h"
      - in general:
          wildcard package import java.a.b.* → "jxx.a.b.h"
          single-type import   java.a.b.T   → "jxx.a.b.T.h"
  • Package → C++ namespace
  • Primitive type mapping to Java-like C++ types (defaults):
      byte→jbyte, short→jshort, int→jint, long→jlong, boolean→jbool,
      char→jchar, float→jfloat, double→jdouble, void→void
    Override via --primitive-map key=value (repeatable).
  ...
  • Exceptions: throw new X → throw X(...); try/catch; 'throws' via macro/comment/off;
                untyped/multi-catch → catch(Exception&)
  • synchronized(expr) → SYNC_MACRO(expr) { ... } (default macro: JXX_SYNCHRONZIED)
  • Enums: simple → enum class + companion meta(values/valueOf);
           rich → class-backed + static values/valueOf and call-site rewrite
...
# Default primitive mapping (override via --primitive-map)
JAVA_TO_CPP_TYPES = {
    "byte": "jbyte",
    "short": "jshort",
    "int": "jint",
...
class Transpiler:
    def __init__(
        ...
        # Primitive type mapping overrides
    ...
    def emit_includes(self, out: Emit) -> None:
        ...
        if not self.no_default_includes:
            ordered.extend(["<iostream>", "<vector>", "<array>"])
            if self.string_policy == "std":
                ordered.append("<string>")

        # Dedup local list; include_written dedups overall
        seen = set()
        for inc in ordered:
    ...
    def gather_usings(self, tree) -> None:
        ...
        for imp in (tree.imports or []):
            ...
            if imp.wildcard:
                ...
                inc = f"jxx.{'.'.join(segs)}.h" if segs else "jxx.h"
                ...
                if getattr(a, "wildcard", False):
                    if a.pattern_type is not None:
                        ...
                        if getattr(a, "kind", None) == "super":
                            acc.append(f"{b} /* ? super */")
                        else:
                            acc.append(f"{b} /* ? extends */")
                    else:
                        acc.append("/* ? */")
    ...
    def emit_expression(self, e) -> str:
        ...
        if tn == "ThrowStatement":
            ex = e.expression
            # Don't use new macro in throw
            if type(ex).__name__ == "ClassCreator":
                name = self._map_ref_or_basic(ex.type)
                args = [self.emit_expression(a) for a in (ex.arguments or [])]
                return f"throw {name}({', '.join(args)})"
            return f"throw {self.emit_expression(ex)}"
    ...
    # ---- statements (includes byte[] locals rule) ----
    def emit_statement(self, s, out: Emit) -> None:
        ...
        if tn == "IfStatement":
            ...
            if s.else_statement is not None:
                out.write("else {")
                out.enter(); self.emit_statement(s.else_statement, out); out.exit(); out.write("}")
            return

        if tn == "WhileStatement":
            out.write(f"while ({self.emit_expression(s.condition)}) {{")
            out.enter(); self.emit_statement(s.body, out); out.exit(); out.write("}")
            return
        ...
        if tn == "ForStatement":
            ...
            out.write(f"for ({', '.join(init_parts)}; {cond}; {upd}) {{")
            out.enter(); self.emit_statement(s.body, out); out.exit(); out.write("}")
            return

        if tn == "TryStatement":
            out.write("try {")
            out.enter(); self.emit_statement(s.block, out); out.exit(); out.write("}")
            for c in (s.catches or []):
                ...
                if not tlist or len(tlist) > 1:
                    ...
                    if tlist:
                        try:
                            listed = " | ".join(self._map_non_byte(t) for t in tlist)
                        except Exception:
                    ...
                    out.enter(); self.emit_statement(c.block, out); out.exit(); out.write("}")
            if s.finally_block is not None:
                out.write("// finally { ... }")
                out.write("{")
                out.enter(); self.emit_statement(s.finally_block, out); out.exit(); out.write("}")
            return
        ...
        if tn == "SynchronizedStatement":
            ...
            out.write(f"{self.sync_macro}({le}) {{")
            out.enter(); self.emit_statement(s.block, out); out.exit(); out.write("}")
            return

        if tn == "SwitchStatement":
            out.write(f"switch ({self.emit_expression(s.expression)}) {{")
            out.enter()
    ...
    def emit_field(self, f, out: Emit, ref_fields_accum: List[Tuple[str, str]]):
        for d in (f.declarators or []):
            base, d0 = _base_type_name_and_dims(f.type)
            extra = len(getattr(d, "dimensions", []) or [])
            total = d0 + extra
            name = d.name
    ...
    def _render_type_params(self, tparams) -> str:
        ...
        for tp in tparams:
            ...
            if getattr(tp, "bounds", None):
                bound = " /* extends " + ", ".join(self._map_ref_or_basic(b) for b in tp.bounds) + " */"
            parts.append(f"typename {tp.name}{bound}")
        return "template<" + ", ".join(parts) + ">"

    def _inline_in_header(self, owner_tpl: bool, m) -> bool:
        return bool(owner_tpl or getattr(m, "type_parameters", None))
    ...
    def _method_signature(self, m, class_name: str, in_interface: bool) -> Tuple[str, bool]:
        ...
        if not is_ctor:
            if m.return_type is not None:
                ...
                if base == "byte" and total >= 1:
                    ...
                    ret = self._map_non_byte(m.return_type, extra_dims=md)
            else:
                ret = "void"
        else:
            ret = ""
        ...
        for p in (m.parameters or []):
            ...
            if b == "byte" and total >= 1:
                ...
                params.append(f"{self._map_non_byte(p.type)} {p.name}")
    ...
    def emit_method_header_or_body(self, m, out: Emit, class_name: str, in_interface: bool, owner_tpl: bool, emit_body: bool) -> None:
        ...
        sig, _ = self._method_signature(m, class_name, in_interface)
    ...
    def _emit_simple_enum_with_helpers(self, en, out: Emit) -> None:
        ...
        out.write(f"struct {name}__meta {{")
        out.enter()
        ...
        out.write(f"static inline {name} valueOf({self._string_param_t()} s) {{")
        out.enter()
        for c in consts:
            out.write(f"if (s == {self._string_lit('"' + c + '"')}) return {name}::{c};")
    ...
    def _emit_rich_enum_header(self, en, out: Emit, emit_bodies_in_header: bool) -> None:
        ...
        if getattr(en, "implements", None):
            for i in en.implements:
                bases.append(self._map_non_byte(i))
        clause = (" : " + ", ".join(f"public {b}" for b in bases)) if bases else ""
        out.write(f"class {name}{clause} {{")
        out.enter()
        out.write("public:")
        out.enter()
        consts = [c.name for c in (en.constants or [])]
    ...
    def emit_type_header(self, t, out: Emit) -> None:
        ...
        if tn not in ("ClassDeclaration", "InterfaceDeclaration"):
            out.write(f"/* TODO: Unhandled type: {tn} */")
            return
        ...
        if getattr(t, "extends", None):
            ...
            for b in ext:
                bases.append(self._map_non_byte(b))
        if getattr(t, "implements", None):
            for i in t.implements:
                bases.append(self._map_non_byte(i))
        clause = (" : " + ", ".join(f"public {b}" for b in bases)) if bases else ""

        out.write(f"{kw} {name}{clause} {{")
        out.enter()
    ...
    def emit_type_source(self, t, out: Emit, ns_stack: List[str], type_stack: List[str]) -> None:
        ...
        if tn == "EnumDeclaration":
            if not self._is_simple_enum(t):
                name = t.name
                fq = self._qstack(type_stack + [name])
                consts = [c.name for c in (t.constants or [])]
                out.write(f"std::array<{fq}, {len(consts)}> {fq}::values() {{")
                out.enter(); out.write(f"return {{ {', '.join(f'{fq}::{c}' for c in consts)} }};"); out.exit(); out.write("}")
                out.write(f"{fq} {fq}::valueOf({self._string_param_t()} s) {{")
                out.enter()
                for c in consts:
                    out.write(f"if (s == {self._string_lit('"' + c + '"')}) return {fq}::{c};")
                out.write(f"throw {self.enum_valueof_exception}(\"No enum const {name}.\" /* + s */);")
                out.exit(); out.write("}")
            for m in (t.body or []):
    ...
    # ---- header guard computation & unit emission ----
    def _compute_guard_from_path(self, header_path: str) -> str:
        base = header_path.replace('\\', '/').upper()
        macro = []
        for ch in base:
            macro.append(ch if ch.isalnum() else '_')
        s = ''.join(macro)
        if not s.endswith('_H') and not s.endswith('_HPP'):
            s += '_H'
        return s

    def transpile_unit(self, java_src: str, mode: str = "monolith", header_rel_include: Optional[str] = None,
                      header_guard_name: Optional[str] = None) -> Tuple[str, str]:
        if javalang is None:
            raise RuntimeError("javalang is required. pip install javalang")
        tree = javalang.parse.parse(java_src)

        self.enums_by_java_qual.clear()
        self.enums_by_unqual.clear()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Java → C++17 AST transpiler (dir mode, header/source split, import includes, primitive mapping)")
    ...
    # Primitive mapping overrides
    ap.add_argument('--primitive-map', action='append', default=[],
                    help='Override primitive type mapping (repeatable), e.g. --primitive-map int=jint --primitive-map boolean=jbool')
    ...
    # Header options
    ap.add_argument('--no-pragma-once', action='store_true', help='Disable #pragma once in generated headers (useful with guards)')
    ap.add_argument('--header-guards', action='store_true', help='Emit header guards (#ifndef/#define/#endif) in headers')
    ap.add_argument('--guard-name', default=None, help='Override macro name for header guard (applied to all headers)')
    ...
    ap.add_argument('--new-macro-style', choices=['template', 'curried'], default='template',
                    help='How to render new: template → JXX_NEW<Foo>(args), curried → JXX_NEW(Foo)(args)')
    ap.add_argument('--new-macro', default='JXX_NEW', help='Macro name used to replace new (default: JXX_NEW)')
...
def handle_single_file_pair(tp: Transpiler, in_path: str, out_root: str) -> Tuple[str, str]:
    ...
    guard_name = tp.guard_name or (tp._compute_guard_from_path(header_path) if tp.header_guards else None)
    text_h, _ = tp.transpile_unit(src, mode='header', header_guard_name=guard_name)
