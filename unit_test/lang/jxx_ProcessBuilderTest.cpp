...
TEST(ProcessBuilderTest, CapturesSeparateStandardError) {
#ifdef _WIN32
    // Keep the redirection operator adjacent to the payload. Otherwise,
    // cmd.exe includes the separating space in the text written by echo.
    auto command = shellCommand("echo jxx_process_error>&2");
#else
    ...
    auto errorOutput = readAll(process->getErrorStream());
#ifdef _WIN32
    // cmd.exe echo appends CRLF. Remove only that known platform suffix.
    if (errorOutput.size() >= 2 &&
        errorOutput.compare(errorOutput.size() - 2, 2, "\r\n") == 0) {
        errorOutput.resize(errorOutput.size() - 2);
    }
#endif

    EXPECT_EQ(errorOutput, "jxx_process_error");
}
...
TEST(ProcessBuilderTest, AppendRedirectPreservesExistingData) {
    ...
    value->redirectOutput(
        jxx::lang::ProcessBuilder::Redirect::appendTo(file));
...
TEST(ProcessTest, ConcurrentWaitersObserveSameExitCode) {
#ifdef _WIN32
    auto process = builder(shellCommand("exit /b 7"))->start();
#else
    ...
    std::thread waiterOne([&] { first.store(process->waitFor()); });
    std::thread waiterTwo([&] { second.store(process->waitFor()); });
